...
class Personnel(Occupant):
  ...
  def __init__(self, _nom, _prenom, _age, _role):
    ...
    self.role = _role
    print('>> The {0} {1} {2} has {3} years old and was added to the personal register'.format(self.role, self.nom.upper(), self.prenom.capitalize(), self.age))

  def __repr__(self):
    return '{self.__class__.__name__}({self.nom}, {self.prenom}, {self.age}, {self.role})'.format(self=self)

  def __str__(self):
    return '>> {1} {2} is a {0} {3} years old, was added'.format(self.role, self.nom.upper(), self.prenom.capitalize(), self.age)
